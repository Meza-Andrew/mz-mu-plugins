...
if (!function_exists('meza_sync_shared_project_field_group_fields')) {
    ...
    function meza_shared_project_field_tree_is_complete(array $expected_fields, array $existing_fields, string $expected_parent_key): bool
    {
        $existing_fields_by_key = [];
        foreach ($existing_fields as $existing_field) {
            if (!is_array($existing_field)) {
        ...
        foreach ($expected_fields as $expected_field) {
            ...
            if ($expected_key !== '') {
                $expected_field_keys[$expected_key] = true;
            }
        }

        foreach ($existing_fields_by_key as $existing_key => $existing_field) {
            if (!isset($expected_field_keys[$existing_key])) {
                return false;
            }
        }
        ...
        foreach ($expected_fields as $expected_field) {
            ...
            if ((string) ($existing_field['key'] ?? '') !== $expected_key) {
                return false;
            }

            if ((string) ($existing_field['parent'] ?? '') !== $expected_parent_key) {
                return false;
            }

            $expected_sub_fields = isset($expected_field['sub_fields']) && is_array($expected_field['sub_fields'])
                ? array_values($expected_field['sub_fields'])
                : [];
            if (!meza_shared_project_field_tree_is_complete(
                $expected_sub_fields,
            ...
            $expected_layouts = isset($expected_field['layouts']) && is_array($expected_field['layouts'])
                ? array_values($expected_field['layouts'])
                : [];
            $existing_layouts_by_key = meza_index_acf_layouts_by_key((array) ($existing_field['layouts'] ?? []));
            $expected_layout_keys = [];
            foreach ($expected_layouts as $expected_layout) {
                if (!is_array($expected_layout)) {
                    continue;
                }

                $layout_index_key = meza_acf_layout_index_key($expected_layout);
                if ($layout_index_key === '') {
                    continue;
                }

                $expected_layout_keys[$layout_index_key] = true;

                $existing_layout = $existing_layouts_by_key[$layout_index_key] ?? null;
                if (!is_array($existing_layout)) {
                    return false;
                }

                $layout_parent_key = (string) ($expected_layout['key'] ?? '');
                if ($layout_parent_key === '') {
                    $layout_parent_key = $expected_key;
                }

                $expected_layout_sub_fields = isset($expected_layout['sub_fields']) && is_array($expected_layout['sub_fields'])
                    ? array_values($expected_layout['sub_fields'])
                    : [];
                if (!meza_shared_project_field_tree_is_complete(
                    $expected_layout_sub_fields,
                    (array) ($existing_layout['sub_fields'] ?? []),
                    $layout_parent_key
                )) {
                    return false;
                }
            ...
            foreach ($existing_layouts_by_key as $existing_layout_key => $existing_layout) {
                if (!isset($expected_layout_keys[$existing_layout_key])) {
                    return false;
                }
            }
        }
    ...
    function meza_shared_project_field_group_definition_tree_is_complete(array $definition): bool
    {
        if (
        ...
        $field_group_key = (string) ($field_group['key'] ?? ($definition['key'] ?? ''));
        if ($field_group_key === '') {
            return false;
        }
        ...
        return meza_shared_project_field_tree_is_complete(
            $expected_fields,
            (array) acf_get_fields($field_group_id),
            $field_group_key
        );
    }

    function meza_shared_project_field_group_definition_trees_are_complete(array $definitions): bool
    {
        foreach ($definitions as $definition) {
            if (!is_array($definition)) {
                continue;
            ...
            if (!meza_shared_project_field_group_definition_tree_is_complete($definition)) {
                return false;
            }
        }
    ...
    function meza_sync_shared_project_field_group_fields(): void
    {
        ...
        if (
            (string) get_option(meza_get_shared_project_field_group_fields_sync_option_name(), '') === $version
...
if (!function_exists('meza_assign_acf_field_tree_parent_keys')) {
    function meza_assign_acf_field_tree_parent_keys(array $field, string $parent_key): array
    {
        if ($parent_key !== '') {
        ...
        $child_parent_key = $field_key !== '' ? $field_key : $parent_key;
        ...
        if (isset($field['sub_fields']) && is_array($field['sub_fields'])) {
            foreach ($field['sub_fields'] as $index => $sub_field) {
                ...
                $field['sub_fields'][$index] = meza_assign_acf_field_tree_parent_keys($sub_field, $child_parent_key);
            }
        }
        ...
        if (isset($field['layouts']) && is_array($field['layouts'])) {
            foreach ($field['layouts'] as $layout_index => $layout) {
                ...
                if ($child_parent_key !== '') {
                    $layout['parent'] = $child_parent_key;
                }

                $layout_key = (string) ($layout['key'] ?? '');
                $layout_parent_key = $layout_key !== '' ? $layout_key : $child_parent_key;
                ...
                if (isset($layout['sub_fields']) && is_array($layout['sub_fields'])) {
                    foreach ($layout['sub_fields'] as $sub_field_index => $sub_field) {
                        ...
                        $layout['sub_fields'][$sub_field_index] = meza_assign_acf_field_tree_parent_keys(
                            $sub_field,
                            $layout_parent_key
                        );
                    }
...
if (!function_exists('meza_index_acf_layouts_by_key')) {
    function meza_index_acf_layouts_by_key(array $layouts): array
    {
        $indexed = [];
        foreach ($layouts as $layout) {
            ...
            $layout_key = (string) ($layout['key'] ?? '');
            if ($layout_key !== '') {
                $indexed[$layout_key] = $layout;
                continue;
            }

            $layout_name = (string) ($layout['name'] ?? '');
            if ($layout_name !== '') {
                $indexed['name:' . $layout_name] = $layout;
            }
        }
...
if (!function_exists('meza_acf_layout_index_key')) {
    function meza_acf_layout_index_key(array $layout): string
    {
        $layout_key = (string) ($layout['key'] ?? '');
        if ($layout_key !== '') {
            return $layout_key;
        ...
        $layout_name = (string) ($layout['name'] ?? '');
        return $layout_name !== '' ? 'name:' . $layout_name : '';
    }
}

if (!function_exists('meza_sync_acf_field_tree')) {
    function meza_sync_acf_field_tree(array $expected_fields, array $existing_fields, string $parent_key): bool
    {
        $existing_fields_by_key = meza_index_acf_fields_by_key($existing_fields);
        $did_update = false;
        ...
        foreach (array_values($expected_fields) as $menu_order => $expected_field) {
            ...
            $expected_field['parent'] = $parent_key;
            $expected_field['menu_order'] = (int) $menu_order;
            $expected_field = meza_assign_acf_field_tree_parent_keys($expected_field, $parent_key);
            ...
            $expected_sub_fields = isset($expected_field['sub_fields']) && is_array($expected_field['sub_fields'])
                ? array_values($expected_field['sub_fields'])
                : [];
            $did_update = meza_sync_acf_field_tree(
                $expected_sub_fields,
                (array) ($existing_field['sub_fields'] ?? []),
                $expected_key
            ) || $did_update;
            ...
            $existing_layouts_by_key = meza_index_acf_layouts_by_key((array) ($existing_field['layouts'] ?? []));
            $expected_layout_keys = [];
            foreach ($expected_layouts as $layout) {
                if (!is_array($layout)) {
                    continue;
                }

                $layout_index_key = meza_acf_layout_index_key($layout);
                if ($layout_index_key === '') {
                    continue;
                }
                ...
                $layout_parent_key = (string) ($layout['key'] ?? '');
                if ($layout_parent_key === '') {
                    $layout_parent_key = $expected_key;
                }

                $expected_layout_sub_fields = isset($layout['sub_fields']) && is_array($layout['sub_fields'])
                    ? array_values($layout['sub_fields'])
                    : [];
                $existing_layout = $existing_layouts_by_key[$layout_index_key] ?? [];
                $did_update = meza_sync_acf_field_tree(
                    $expected_layout_sub_fields,
                    (array) ($existing_layout['sub_fields'] ?? []),
                    $layout_parent_key
            ...
            foreach ($existing_layouts_by_key as $existing_layout_key => $existing_layout) {
                ...
                $layout_parent_key = (string) ($existing_layout['key'] ?? '');
                if ($layout_parent_key === '') {
                    $layout_parent_key = $expected_key;
                }
        ...
        foreach ($existing_fields_by_key as $existing_key => $existing_field) {
            $expected_field = null;
            foreach ($expected_fields as $candidate) {
                if (is_array($candidate) && (string) ($candidate['key'] ?? '') === $existing_key) {
...
if (!function_exists('meza_sync_builtin_field_group_fields')) {
    function meza_sync_builtin_field_group_fields(array $definition): bool
    {
        ...
        return meza_sync_acf_field_tree(
            $expected_fields,
            $existing_fields,
