...
function meza_is_rest_api_service_user($user = null): bool
{
    return function_exists('meza_rest_api_service_role_key')
        && meza_user_has_any_role($user, [meza_rest_api_service_role_key()]);
}

function meza_is_content_editor_user($user = null): bool
{
    return function_exists('meza_content_editor_role_key')
        && meza_user_has_any_role($user, [meza_content_editor_role_key()]);
}

function meza_is_shop_manager_user($user = null): bool
{
    return function_exists('meza_shop_manager_role_key')
        && meza_user_has_any_role($user, [meza_shop_manager_role_key()]);
}

function meza_can_access_profile_application_passwords($user = null): bool
{
    return meza_user_has_any_role($user, [
        'administrator',
        function_exists('meza_rest_api_service_role_key') ? meza_rest_api_service_role_key() : 'rest_api_service',
    ]);
}

function meza_should_restore_appearance_group_separator($user = null): bool
{
    return meza_can_view_settings_tools_submenu_items($user)
...
function meza_get_rest_api_service_security_menu_slug(): string
{
    return 'meza-rest-api-service-security';
}

function meza_get_rest_api_service_two_factor_menu_slug(): string
{
    return 'meza-rest-api-service-security-two-factor';
}
...
function meza_get_rest_api_service_two_factor_url(): string
{
    return admin_url('admin.php?page=aiowpsec_two_factor_auth_user');
}
...
function meza_get_aios_password_strength_menu_slug(): string
{
    return 'aiowpsec_tools';
}

function meza_get_aios_password_strength_menu_url_slug(): string
{
    return 'admin.php?page=aiowpsec_tools&tab=password-tool';
}
...
function meza_can_access_limited_aios_security_menu($user = null): bool
{
    return meza_is_aios_plugin_active()
        ...
        && !meza_is_rest_api_service_user($user)
        && !meza_is_seo_manager_user($user)
        && !meza_is_administrator_user($user);
}

function meza_can_access_rest_api_service_security_menu($user = null): bool
{
    return meza_is_aios_plugin_active()
        && meza_is_rest_api_service_user($user)
        && meza_is_logged_in_wordpress_user($user);
...
function meza_get_settings_admin_page_menu_slug(string $page_slug): string
{
    ...
    return $page_slug !== ''
        ? 'options-general.php?page=' . $page_slug
        : '';
}
...
function meza_get_site_manager_default_settings_menu_slug(): string
{
    ...
    return $default_slug !== ''
        ? meza_get_settings_admin_page_menu_slug($default_slug)
        : 'options-general.php';
}
...
add_action('admin_init', function (): void {
    ...
    if ($page === 'meza-site-manager-security-two-factor') {
        wp_safe_redirect(meza_get_site_manager_aios_two_factor_url());
        exit;
    }

    if ($page === 'meza-site-manager-security-password-strength') {
        wp_safe_redirect(admin_url('admin.php?page=aiowpsec_tools'));
        exit;
    }
...
add_filter('parent_file', function ($parent_file) {
    ...
    if (
        function_exists('meza_is_aios_user_two_factor_request')
        && meza_is_aios_user_two_factor_request()
        && meza_can_access_rest_api_service_security_menu(wp_get_current_user())
    ) {
        return meza_get_rest_api_service_security_menu_slug();
    }
...
add_filter('submenu_file', function ($submenu_file) {
    ...
    if (
        ...
        return meza_get_rest_api_service_two_factor_menu_slug();
    }
...
function meza_enforce_yoast_admin_menu_state(): void
{
    ...
    $ordered_labels = meza_is_content_editor_user($user)
        ? ['Tools']
        : ['Tools', 'Settings'];
    ...
    foreach ($menu as &$item) {
        ...
        $item[2] = $parent_slug;
        $item[0] = meza_get_yoast_admin_menu_label();
        if (isset($item[3])) {
            $item[3] = meza_get_yoast_admin_menu_label();
        }
        $item[6] = meza_get_yoast_admin_menu_icon();
        break;
    }
    unset($item);
}
...
add_action('admin_init', function (): void {
    ...
    // Yoast settings uses hash-based routes that never reach PHP, so redirecting
    // wpseo_page_settings here creates a self-redirect loop for deep links.
    wp_safe_redirect(meza_get_default_yoast_settings_url($user));
    exit;
}, 2);
...
function meza_render_rest_api_service_aios_security_page(): void
{
    if (!meza_can_access_rest_api_service_security_menu(wp_get_current_user())) {
        wp_die(esc_html__('Sorry, you are not allowed to access this page.'));
    }

    wp_safe_redirect(meza_get_rest_api_service_two_factor_url());
    exit;
}

function meza_render_rest_api_service_aios_two_factor_redirect_page(): void
{
    if (!meza_can_access_rest_api_service_security_menu(wp_get_current_user())) {
        wp_die(esc_html__('Sorry, you are not allowed to access this page.'));
    }
...
function meza_force_site_manager_aios_menu_access(): void
{
    ...
    if (!meza_admin_menu_has_top_level_slug(meza_get_site_manager_aios_security_menu_slug())) {
        add_menu_page(
            'Security',
            'Security',
            'read',
...
function meza_force_rest_api_service_aios_menu_access(): void
{
    if (!meza_can_access_rest_api_service_security_menu(wp_get_current_user())) {
        return;
    }
    ...
    add_menu_page(
        ...
        'Security',
        'read',
        meza_get_rest_api_service_security_menu_slug(),
        'meza_render_rest_api_service_aios_security_page',
        'dashicons-shield',
        81
    );
    ...
    add_submenu_page(
        ...
        'Two Factor Auth',
        'read',
        meza_get_rest_api_service_two_factor_menu_slug(),
        'meza_render_rest_api_service_aios_two_factor_redirect_page'
    );
}
add_action('admin_menu', 'meza_force_rest_api_service_aios_menu_access', PHP_INT_MAX);
add_action('admin_menu_editor-menu_replaced', 'meza_force_rest_api_service_aios_menu_access', PHP_INT_MAX);
    ...
    foreach (['aiowpsec', meza_get_site_manager_aios_security_menu_slug(), meza_get_limited_aios_password_strength_parent_slug()] as $parent_slug) {
        ...
        foreach ($submenu[$parent_slug] as $item) {
            ...
            if (in_array($slug, [
                ...
                meza_get_aios_password_strength_menu_slug(),
                meza_get_limited_aios_password_strength_menu_slug(),
            ], true)) {
...
function meza_ensure_allowed_yoast_admin_menus(): void
{
    ...
    meza_insert_missing_seo_manager_top_level_menu_item(
        [meza_get_yoast_admin_menu_label(), 'read', 'wpseo_dashboard', meza_get_yoast_admin_menu_label(), 'menu-top toplevel_page_wpseo_dashboard', 'toplevel_page_wpseo_dashboard', meza_get_yoast_admin_menu_icon()],
    ...
    if (!$has_yoast_settings && !meza_is_content_editor_user($user)) {
...
add_action('admin_init', function (): void {
    ...
    if (
        ...
        if (function_exists('meza_get_shared_project_acf_options_page_menu_slug')) {
            $target_slug = meza_get_shared_project_acf_options_page_menu_slug($page);
            if ($target_slug !== '' && $target_slug !== 'admin.php?page=' . $page) {
                wp_safe_redirect(admin_url($target_slug));
                exit;
            }
        }

        $parent_slug = function_exists('meza_get_shared_project_acf_options_page_parent_slug')
            ? meza_get_shared_project_acf_options_page_parent_slug($page)
            : '';
    ...
    if ($pagenow === 'options-general.php') {
        ...
        if ($page === '' || !in_array($page, $allowed_settings_pages, true)) {
            ...
            $target_slug = $target_page !== ''
                ? meza_get_settings_admin_page_menu_slug($target_page)
                : '';
            wp_safe_redirect($target_slug !== '' ? admin_url($target_slug) : admin_url());
            exit;
        }
    }
...
function meza_get_standardized_submenu_utility_label(array $item, string $parent_slug = ''): string
{
    ...
    if (
        ...
        && (
            str_contains($slug, 'migration')
            || preg_match('/\bmigration\b/i', $title) === 1
        )
    ) {
        return 'Migration';
    }
...
function meza_get_keyword_dashicon_for_top_level_menu_item(string $label, string $slug = ''): string
{
    ...
    if (str_contains($haystack, 'yoast') || str_contains($haystack, 'wpseo') || str_contains($haystack, 'wordpress-seo')) {
        return meza_get_yoast_admin_menu_icon();
    }
...
function meza_get_keyword_top_level_menu_label(string $label, string $slug = ''): string
{
    ...
    if (str_contains($haystack, 'yoast') || str_contains($haystack, 'wpseo') || str_contains($haystack, 'wordpress-seo')) {
        return meza_get_yoast_admin_menu_label();
    }
...
function meza_post_type_is_visible_in_admin_menu(string $post_type): bool
{
    ...
    if ($post_type === 'post') {
        $visibility_cache[$post_type] = function_exists('meza_are_posts_enabled')
...
function meza_normalize_admin_plugin_menus(): void
{
    ...
    foreach ($menu as $index => &$item) {
        ...
        if ($is_site_kit) {
            $item[0] = 'Analytics';
            if (isset($item[3])) $item[3] = 'Analytics';
        ...
        if ($is_yoast) {
            $item[0] = meza_get_yoast_admin_menu_label();
            if (isset($item[3])) $item[3] = meza_get_yoast_admin_menu_label();
            $item[6] = meza_get_yoast_admin_menu_icon();
            continue;
        }
...
function meza_get_dashboard_group_end_index(array $menu): ?int
{
    ...
    for ($i = $dashboard_index + 1, $count = count($menu); $i < $count; $i++) {
        ...
        $is_dashboard_utility = str_contains($slug, 'googlesitekit')
            ...
            || str_contains($slug, 'wordpress-seo')
            || in_array($title, ['analytics', 'web analytics', 'site kit', 'site kit by google', 'seo'], true);
...
add_filter('admin_body_class', function (string $classes): string {
    ...
    if (meza_is_content_editor_user(wp_get_current_user())) {
        $classes .= ' meza-content-manager';
    }
...
add_action('admin_head', function (): void {
    ...
    echo '<style id="meza-content-manager-php-error-menu-reset">' .
        'body.meza-content-manager.php-error #adminmenuback,' .
        'body.meza-content-manager.php-error #adminmenuwrap{' .
        'margin-top:0!important;' .
        '}' .
        '</style>';
}, 1003);
...
?>
        ...
        (() => {
            const removeActiveMenuIconStyles = () => {
                document.querySelectorAll(
                    ...
                    // SVG menu icons store their artwork in an inline background-image,
                    // so stripping the whole style attribute makes them disappear.
                    if (icon.classList.contains('svg')) return;
    ...
    <style id="meza-reset-active-top-level-admin-menu-style">
        ...
        #adminmenu li.wp-menu-open > a.menu-top .wp-menu-image.svg {
...
if (!function_exists('meza_is_admin_chrome_exempt_screen')) {
    ...
    function meza_is_current_acf_admin_or_options_screen(?WP_Screen $screen = null): bool
    {
        ...
        return in_array($page, ['branding', 'business-information', 'crm', 'ecommerce'], true);
    }
    ...
    function meza_is_preferred_plugin_admin_screen(?WP_Screen $screen = null): bool
    {
        ...
        if (function_exists('meza_is_woocommerce_admin_screen') && meza_is_woocommerce_admin_screen($screen)) {
            return true;
        }
    ...
    function meza_is_admin_chrome_exempt_screen(): bool
    {
        ...
        if (function_exists('get_current_screen')) {
            ...
            if ($screen instanceof WP_Screen) {
                ...
                if (in_array($screen_base, ['edit-tags', 'term'], true)) {
                    return true;
                }
        ...
        if (in_array($php_self, ['edit-tags.php', 'term.php'], true)) {
            return true;
        }
...
function meza_is_settings_utility_menu_item($item): bool
{
    ...
    $is_yoast = str_contains($slug, 'wpseo')
        || str_contains($slug, 'wordpress-seo')
        || in_array($label, ['seo', 'yoast seo'], true);

    return $is_acf || $is_mail || $is_security || $is_backups || $is_make || $is_yoast;
}
...
function meza_enforce_site_manager_settings_top_level_target(): void
{
    ...
    foreach ($menu as &$item) {
        ...
        $slug = (string) ($item[2] ?? '');
        $label = strtolower(trim(wp_strip_all_tags((string) ($item[0] ?? ''))));
        $is_settings_target = in_array($slug, ['options-general.php', meza_get_site_manager_default_settings_menu_slug()], true)
            || $label === 'settings'
            || str_contains($slug, 'menu_editor')
            || str_contains($slug, 'menu-editor')
            || str_contains($slug, 'admin-menu-editor');
...
function meza_enforce_site_manager_security_submenu(): void
{
    if (!meza_is_site_manager_user(wp_get_current_user()) || !meza_is_aios_plugin_active()) {
        return;
    }
...
// Keep ACF, Backups, Security, Yoast SEO, Mail, and Make in their own utility group below Settings.
...
function meza_prune_empty_top_level_admin_menu_groups(): void
{
    ...
    $protected_top_level_slugs = [
        ...
        meza_get_integrations_settings_menu_slug(),
    ];
...
function meza_restore_settings_utility_group_separator(): void
{
    ...
    $next_item = $menu[$settings_index + 1] ?? null;
    if ($next_item === null || $is_separator($next_item)) {
        return;
    }

    $has_following_non_separator = false;

    for ($index = $settings_index + 1, $count = count($menu); $index < $count; $index++) {
        if ($is_separator($menu[$index] ?? null)) {
            continue;
        }

        $has_following_non_separator = true;
        break;
    }

    if (!$has_following_non_separator) {
        return;
    }

    array_splice($menu, $settings_index + 1, 0, [[
        '',
        'read',
        'separator-meza-settings-utilities',
        '',
...
function meza_group_site_manager_appearance_and_fallback_menus(): void
{
    ...
    $is_appearance_group_item = static function ($item): bool {
        ...
        return in_array($slug, ['themes.php', 'plugins.php', 'users.php', 'tools.php', 'meza-business-settings'], true)
            || $slug === 'options-general.php'
            || in_array($label, ['business', 'appearance', 'plugins', 'users', 'tools', 'settings'], true);
    };
...
{
    if (!meza_should_restore_appearance_group_separator(wp_get_current_user())) {
        return;
    }
...
function meza_enforce_restricted_top_level_utility_menus(): void
{
    ...
    if (
        meza_is_seo_manager_user(wp_get_current_user())
        || meza_is_content_editor_user(wp_get_current_user())
    ) {
        return;
    }
...
function meza_filter_rest_api_service_admin_menu(): void
{
    if (!meza_is_rest_api_service_user(wp_get_current_user())) {
        return;
    }
    ...
    if (is_array($menu)) {
        ...
        foreach ($menu as $item) {
            ...
            $slug = (string) ($item[2] ?? '');
            if ($slug === 'index.php' && $dashboard_item === null) {
                $dashboard_item = $item;
                continue;
            }

            if (in_array($slug, ['profile.php', 'users.php'], true) && $profile_item === null) {
                $profile_item = $item;
                continue;
            }

            if ($slug === meza_get_rest_api_service_security_menu_slug() && $security_item === null) {
                $security_item = $item;
            }
        }
        ...
        if (is_array($dashboard_item)) {
            $rebuilt_menu[] = $dashboard_item;
        }

        if (is_array($profile_item)) {
            $profile_item[0] = 'Profile';
            $profile_item[2] = 'profile.php';
            if (isset($profile_item[3])) {
                $profile_item[3] = 'Profile';
            }
        }

        $utility_group = [];
        if (is_array($profile_item)) {
            $utility_group[] = $profile_item;
        }
        if (is_array($security_item)) {
            $utility_group[] = $security_item;
        }

        if (!empty($rebuilt_menu) && !empty($utility_group)) {
            $rebuilt_menu[] = [
                '',
                'read',
                'separator-meza-rest-api-service-utilities',
                '',
                'wp-menu-separator',
            ];
        }

        foreach ($utility_group as $item) {
            $rebuilt_menu[] = $item;
        }
    ...
    foreach ($submenu as $parent_slug => $items) {
        ...
        if ($parent_slug === 'index.php') {
            $submenu[$parent_slug] = array_values(array_filter($items, static function ($item): bool {
                return is_array($item) && ((string) ($item[2] ?? '')) === 'index.php';
            }));
            continue;
        }
        ...
        if (in_array($parent_slug, ['users.php', 'profile.php'], true)) {
            $submenu[$parent_slug] = array_values(array_filter($items, static function ($item): bool {
                return is_array($item) && ((string) ($item[2] ?? '')) === 'profile.php';
            }));
            continue;
        }

        if ($parent_slug === meza_get_rest_api_service_security_menu_slug()) {
            $submenu[$parent_slug] = array_values(array_filter($items, static function ($item): bool {
                return is_array($item) && ((string) ($item[2] ?? '')) === meza_get_rest_api_service_two_factor_menu_slug();
            }));
            continue;
        }
...
add_action('admin_menu', 'meza_filter_rest_api_service_admin_menu', PHP_INT_MAX);

function meza_filter_content_editor_admin_menu(): void
{
    if (!meza_is_content_editor_user(wp_get_current_user())) {
        return;
    }
    ...
    if (is_array($menu)) {
        $menu = array_values(array_filter($menu, static function ($item) use ($blocked_top_level_slugs): bool {
            if (!is_array($item)) {
                return true;
            }

            return !in_array((string) ($item[2] ?? ''), $blocked_top_level_slugs, true);
        }));
        ...
        foreach ($menu as $item) {
            ...
            $slug = (string) ($item[2] ?? '');
            if (in_array($slug, ['profile.php', 'users.php'], true)) {
        ...
        while (!empty($rebuilt_menu)) {
            $last_item = end($rebuilt_menu);
            if (!is_array($last_item) || !str_starts_with((string) ($last_item[2] ?? ''), 'separator')) {
                break;
            }

            array_pop($rebuilt_menu);
        }

        if ($profile_item !== null || $tools_item !== null) {
            $rebuilt_menu[] = [
                '',
                'read',
                'separator-meza-content-editor-utilities',
                '',
                'wp-menu-separator',
            ];
        }
    ...
    foreach ($submenu as $parent_slug => $items) {
        ...
        if (in_array((string) $parent_slug, $blocked_top_level_slugs, true)) {
            unset($submenu[$parent_slug]);
        }
    }

    if (isset($submenu['wpseo_dashboard']) && is_array($submenu['wpseo_dashboard'])) {
        $submenu['wpseo_dashboard'] = array_values(array_filter($submenu['wpseo_dashboard'], static function ($item): bool {
            if (!is_array($item)) {
                return false;
            }

            return in_array((string) ($item[2] ?? ''), ['wpseo_tools', 'admin.php?page=wpseo_tools'], true);
        }));
    }
}
add_action('admin_menu', 'meza_filter_content_editor_admin_menu', PHP_INT_MAX);

function meza_remove_content_editor_separator_between_profile_and_tools(): void
{
    if (!meza_is_content_editor_user(wp_get_current_user())) {
        return;
    }
    ...
    foreach ($menu as $index => $item) {
        ...
        $slug = (string) ($item[2] ?? '');
        if ($slug === 'profile.php' && $profile_index === null) {
            $profile_index = (int) $index;
            continue;
        }

        if ($slug === 'tools.php') {
            $tools_index = (int) $index;
            break;
        }
    }

    if ($profile_index === null || $tools_index === null || $tools_index <= $profile_index + 1) {
        return;
    }

    for ($index = $tools_index - 1; $index > $profile_index; $index--) {
        ...
        $slug = strtolower((string) ($item[2] ?? ''));
        $classes = strtolower((string) ($item[4] ?? ''));
        $is_separator = str_starts_with($slug, 'separator') || str_contains($classes, 'wp-menu-separator');
        if ($is_separator) {
            array_splice($menu, $index, 1);
        }
    }
...
function meza_filter_shop_manager_admin_menu(): void
{
    if (!meza_is_shop_manager_user(wp_get_current_user())) {
        return;
    }
    ...
    if (is_array($menu)) {
        $allowed_lookup = [
            'index.php' => null,
            'edit.php?post_type=product' => null,
            'upload.php' => null,
            'themes.php' => null,
            'profile.php' => null,
            'users.php' => null,
        ...
        $content_group = [];
        foreach (['edit.php?post_type=product', 'upload.php'] as $slug) {
            if (isset($allowed_lookup[$slug]) && is_array($allowed_lookup[$slug])) {
                $content_group[] = $allowed_lookup[$slug];
            }
        }
        ...
        if (!empty($rebuilt_menu) && !empty($content_group)) {
            $rebuilt_menu[] = [
                '',
                'read',
                'separator-meza-shop-manager-content-start',
                '',
                'wp-menu-separator',
            ];
        }
        ...
        $appearance_group = [];
        if (is_array($allowed_lookup['themes.php'])) {
            $appearance_group[] = $allowed_lookup['themes.php'];
        }
        ...
        if (is_array($profile_item)) {
            ...
            $appearance_group[] = $profile_item;
        }

        if ((!empty($rebuilt_menu) || !empty($content_group)) && !empty($appearance_group)) {
            $rebuilt_menu[] = [
                '',
                'read',
                'separator2',
        ...
        foreach ($appearance_group as $item) {
            $rebuilt_menu[] = $item;
        }
    ...
    foreach ($submenu as $parent_slug => $items) {
        ...
        if (!in_array($parent_slug, $allowed_parent_slugs, true)) {
            unset($submenu[$parent_slug]);
        }
    }
}
add_action('admin_menu', 'meza_filter_shop_manager_admin_menu', PHP_INT_MAX);
...
function meza_is_product_admin_available(): bool
{
    return post_type_exists('product');
}

function meza_is_stale_product_admin_request(): bool
{
    if (!is_admin() || meza_is_product_admin_available()) {
        return false;
    }
    ...
    if ($post_type === 'product' && in_array($pagenow, ['edit.php', 'post-new.php', 'post.php'], true)) {
        return true;
    }

    $taxonomy = isset($_GET['taxonomy']) ? sanitize_key(wp_unslash((string) $_GET['taxonomy'])) : '';
    if ($taxonomy !== '' && in_array($taxonomy, ['product_brand', 'product_cat', 'product_tag'], true) && in_array($pagenow, ['edit-tags.php', 'term.php'], true)) {
        return true;
    }

    $page = isset($_GET['page']) ? sanitize_key((string) wp_unslash($_GET['page'])) : '';

    return $page === 'product_attributes';
}

add_action('admin_init', function (): void {
    if (!meza_is_stale_product_admin_request()) {
        return;
    }
...
add_action('admin_init', function (): void {
    if (!is_admin() || !meza_is_shop_manager_user(wp_get_current_user())) {
        return;
    }
    ...
    if (in_array($pagenow, ['users.php', 'user-edit.php', 'user-new.php'], true)) {
...
add_action('admin_menu', function (): void {
    ...
    remove_submenu_page('wpseo_dashboard', 'admin.php?page=wpseo_tools');
    remove_submenu_page('wpseo_dashboard', 'wpseo_bulk-editor');
    remove_submenu_page('wpseo_dashboard', 'admin.php?page=wpseo_bulk-editor');
}, PHP_INT_MAX);

add_action('admin_init', function (): void {
    if (!is_admin() || (function_exists('wp_doing_ajax') && wp_doing_ajax()) || !meza_is_rest_api_service_user(wp_get_current_user())) {
        return;
    }
...
add_filter('woocommerce_prevent_admin_access', function ($prevent_access): bool {
    if (meza_is_rest_api_service_user(wp_get_current_user())) {
        return false;
    }
...
add_action('admin_init', function (): void {
    if (!is_admin() || (function_exists('wp_doing_ajax') && wp_doing_ajax()) || !meza_is_content_editor_user(wp_get_current_user())) {
        return;
    }
    ...
    $page = isset($_GET['page']) ? sanitize_key((string) wp_unslash($_GET['page'])) : '';
    if (
        in_array($pagenow, ['options-general.php', 'themes.php', 'plugins.php', 'users.php', 'user-edit.php', 'user-new.php'], true)
        || in_array($page, [meza_get_business_settings_menu_slug(), meza_get_site_settings_menu_slug(), meza_get_integrations_settings_menu_slug()], true)
    ) {
...
add_filter('wp_is_application_passwords_available_for_user', function (bool $available, $user): bool {
    ...
    return $user instanceof WP_User && meza_can_access_profile_application_passwords($user);
}, 10, 2);

add_action('admin_head-profile.php', function (): void {
    $can_see_two_factor = meza_can_access_profile_two_factor() ? 'true' : 'false';
    $can_see_application_passwords = meza_can_access_profile_application_passwords(wp_get_current_user()) ? 'true' : 'false';
?>
...
function meza_apply_late_admin_menu_mutations(): void
{
    ...
    meza_filter_rest_api_service_admin_menu();
    meza_filter_content_editor_admin_menu();
    meza_filter_shop_manager_admin_menu();
    ...
    meza_enforce_site_manager_settings_submenu();
    meza_enforce_site_manager_security_submenu();
    ...
    meza_position_integrations_settings_menu();
    meza_remove_integrations_settings_menu_for_non_admins();
    meza_position_site_settings_menu();
    ...
    meza_move_dashboard_under_site_menu();
...
function meza_remove_stale_product_admin_menu_items(): void
{
    if (meza_is_product_admin_available()) {
        return;
    }
    ...
    if (is_array($menu)) {
        $menu = array_values(array_filter($menu, static function ($item): bool {
            ...
            return ((string) ($item[2] ?? '')) !== 'edit.php?post_type=product';
        }));
    }
    ...
    foreach ($submenu as $parent_slug => $items) {
        ...
        $filtered_items = array_values(array_filter($items, static function ($item): bool {
            if (!is_array($item)) {
                return false;
            }

            $slug = html_entity_decode((string) ($item[2] ?? ''), ENT_QUOTES, get_bloginfo('charset') ?: 'UTF-8');
            if ($slug === '') {
                return true;
            }
            ...
            if (str_contains($slug, 'page=product_attributes')) {
                return false;
            }
            ...
            return !(
                ...
                || in_array((string) ($query_args['taxonomy'] ?? ''), ['product_brand', 'product_cat', 'product_tag'], true)
            );
        }));
        ...
        $submenu[$parent_slug] = $filtered_items;
    }
}

function meza_restore_expected_post_type_archive_submenus(): void
{
    global $submenu;
    ...
    foreach (get_post_types(['show_ui' => true, 'show_in_menu' => true], 'objects') as $post_type => $post_type_object) {
        ...
        $parent_slug = meza_get_top_level_menu_slug_for_post_type((string) $post_type);
        if ($parent_slug === '' || !isset($submenu[$parent_slug]) || !is_array($submenu[$parent_slug])) {
            continue;
        }

        $plural_label = trim((string) ($post_type_object->labels->name ?? $post_type_object->labels->singular_name ?? $post_type));
        $plural_label = $plural_label !== '' ? $plural_label : ucwords(str_replace(['-', '_'], ' ', (string) $post_type));
        $expected_label = 'All ' . $plural_label;
        $expected_slug = $parent_slug;
        $edit_capability = (string) ($post_type_object->cap->edit_posts ?? 'edit_posts');

        $items = array_values($submenu[$parent_slug]);
        $has_expected_item = false;
        ...
        foreach ($items as &$item) {
            ...
            $item_slug = (string) ($item[2] ?? '');
            if ($item_slug === $expected_slug) {
                $has_expected_item = true;
                break;
            }

            $label = trim(wp_strip_all_tags((string) ($item[0] ?? '')));
            $normalized_label = strtolower($label);
            $normalized_expected_label = strtolower($expected_label);
            $looks_like_archive_item = preg_match('/^all\b/i', $label) === 1
                || $normalized_label === strtolower($plural_label)
                || $normalized_label === $normalized_expected_label;

            if (!$looks_like_archive_item) {
                continue;
            }

            $item[2] = $expected_slug;
            if ($label === '' || preg_match('/^all\b/i', $label) === 1) {
                $item[0] = $expected_label;
                if (isset($item[3])) {
                    $item[3] = $expected_label;
                }
            }
            if (isset($item[1]) && (string) $item[1] === '') {
                $item[1] = $edit_capability;
            }
...
function meza_get_dashboard_updates_submenu_item(): ?array
{
    ...
    return [__('Updates'), $capability, 'update-core.php', __('Updates')];
}
...
function meza_get_dashboard_site_health_submenu_item(): ?array
{
    ...
    return [__('Health'), 'view_site_health_checks', 'site-health.php', __('Health')];
}

function meza_get_dashboard_activity_submenu_item(): ?array
{
    global $submenu;

    $activity_slug = defined('MEZA_ACTIVITY_LOG_PAGE_SLUG') ? (string) MEZA_ACTIVITY_LOG_PAGE_SLUG : '';
    $activity_menu_slug = $activity_slug;
    ...
    if (isset($submenu['index.php']) && is_array($submenu['index.php'])) {
        foreach ($submenu['index.php'] as $item) {
            ...
            $slug = strtolower((string) ($item[2] ?? ''));
            if (
                $slug === strtolower($activity_slug)
                || $slug === 'index.php?page=' . strtolower($activity_slug)
                || $slug === 'admin.php?page=' . strtolower($activity_slug)
            ) {
                $item[2] = $activity_slug;
                return $item;
            }
        }
    }

    if (!function_exists('meza_activity_log_can_view') || !meza_activity_log_can_view()) {
        return null;
    }

    $capability = function_exists('meza_activity_log_get_menu_capability')
        ? meza_activity_log_get_menu_capability()
        : 'edit_others_posts';

    return [__('Activity'), $capability, $activity_menu_slug, __('Activity')];
}

function meza_get_dashboard_business_information_submenu_item(): ?array
{
    return null;
}
...
function meza_filter_dashboard_submenu_items(): void
{
    ...
    $home_item = [__('Home'), 'read', 'index.php', __('Home')];
    $filtered_items = [$home_item];

    // Keep these core/admin utility screens under Dashboard only for users who
    // cannot access the custom Site top-level menu.
    if (!function_exists('meza_can_access_site_settings') || !meza_can_access_site_settings(wp_get_current_user())) {
        ...
        if (is_array($activity_item)) {
            $filtered_items[] = $activity_item;
        }
    }

    $seen_slugs = [];
    $submenu['index.php'] = array_values(array_filter($filtered_items, static function ($item) use (&$seen_slugs): bool {
        if (!is_array($item)) {
            return false;
        }
...
function meza_get_site_settings_analytics_target_slug(): string
{
    if (!function_exists('meza_is_site_kit_plugin_available') || !meza_is_site_kit_plugin_available()) {
        return '';
    }

    $user = wp_get_current_user();
    $can_view_dashboard = function_exists('meza_can_access_site_kit_dashboard')
        && meza_can_access_site_kit_dashboard($user);
    $can_manage_site_kit = function_exists('meza_can_manage_site_kit')
        && meza_can_manage_site_kit($user);
    $can_view_splash = function_exists('meza_can_access_site_kit_splash')
        && meza_can_access_site_kit_splash($user);

    if ($can_view_dashboard || $can_manage_site_kit) {
        return function_exists('meza_get_site_kit_dashboard_menu_slug')
            ? meza_get_site_kit_dashboard_menu_slug()
            : 'admin.php?page=googlesitekit-dashboard';
    }

    if ($can_view_splash) {
        return 'admin.php?page=googlesitekit-splash';
    }

    return '';
}

function meza_get_site_settings_analytics_submenu_item(): ?array
{
    global $menu;

    $target_slug = meza_get_site_settings_analytics_target_slug();
    if ($target_slug === '') {
        return null;
    }
    ...
    return ['Analytics', 'read', $target_slug, 'Analytics'];
}

function meza_get_site_settings_configuration_menu_slug(): string
{
    if (function_exists('meza_get_shared_project_acf_options_page_menu_slug')) {
        $menu_slug = meza_get_shared_project_acf_options_page_menu_slug('content-model');
        if (is_string($menu_slug) && $menu_slug !== '') {
            return $menu_slug;
        }
    }

    return meza_get_settings_admin_page_menu_slug('content-model');
}

function meza_get_site_settings_configuration_submenu_slug(): string
{
    if (function_exists('meza_get_shared_project_acf_options_page_submenu_slug')) {
        $submenu_slug = meza_get_shared_project_acf_options_page_submenu_slug('content-model');
...
function meza_get_site_settings_configuration_submenu_item(): ?array
{
    $capability = function_exists('meza_get_shared_project_acf_options_page_capability')
        ? (string) meza_get_shared_project_acf_options_page_capability('content-model')
        : 'manage_options';
    ...
    return [
        'Content Model',
        $capability,
        meza_get_site_settings_configuration_submenu_slug(),
        'Content Model',
    ];
}
...
function meza_get_site_settings_submenu_target_slug(): string
{
    ...
    foreach (meza_get_current_admin_menu_slug_candidates() as $candidate) {
        ...
        if (
            ...
            return $analytics_target;
        }

        if ($candidate === 'site-health.php') {
            return 'site-health.php';
        }

        if ($candidate === 'update-core.php') {
        ...
        if (
            ...
            && in_array($candidate, [
                $activity_slug,
                'index.php?page=' . $activity_slug,
                'admin.php?page=' . $activity_slug,
            ], true)
        ) {
...
function meza_move_dashboard_under_site_menu(): void
{
    if (!meza_can_access_site_settings(wp_get_current_user())) {
        return;
    }
    ...
    if (!isset($submenu[$site_slug]) || !is_array($submenu[$site_slug])) {
        $submenu[$site_slug] = [];
    }
    ...
    $analytics_target = meza_get_site_settings_analytics_target_slug();
    if ($analytics_target !== '') {
        $consumed_slugs[$analytics_target] = true;
        $consumed_slugs['googlesitekit-dashboard'] = true;
        $consumed_slugs['admin.php?page=googlesitekit-dashboard'] = true;
        $consumed_slugs['googlesitekit-splash'] = true;
        $consumed_slugs['admin.php?page=googlesitekit-splash'] = true;
        $consumed_slugs['meza-web-analytics'] = true;
        $consumed_slugs['mz-web-analytics'] = true;
    }

    $activity_slug = defined('MEZA_ACTIVITY_LOG_PAGE_SLUG') ? (string) MEZA_ACTIVITY_LOG_PAGE_SLUG : '';
    if ($activity_slug !== '') {
        $consumed_slugs[$activity_slug] = true;
        $consumed_slugs['index.php?page=' . $activity_slug] = true;
        $consumed_slugs['admin.php?page=' . $activity_slug] = true;
    }

    foreach ($submenu[$site_slug] as $item) {
        if (!is_array($item)) {
            continue;
        }
    ...
    $submenu[$site_slug] = array_values(array_merge($ordered_items, $site_items));
...
add_action('admin_menu', 'meza_move_dashboard_under_site_menu', PHP_INT_MAX);
add_action('admin_menu_editor-menu_replaced', 'meza_move_dashboard_under_site_menu', PHP_INT_MAX);
add_action('admin_head', 'meza_move_dashboard_under_site_menu', PHP_INT_MAX);

function meza_register_site_settings_real_parent_mappings(): void
{
    if (!meza_can_access_site_settings(wp_get_current_user())) {
        return;
    }

    global $_wp_real_parent_file, $_parent_pages;
    ...
    if ($analytics_target !== '') {
        $mappings[] = 'googlesitekit-dashboard';
        $mappings[] = 'admin.php?page=googlesitekit-dashboard';
        $mappings[] = 'googlesitekit-splash';
        $mappings[] = 'admin.php?page=googlesitekit-splash';
        $mappings[] = $analytics_target;
    }

    if ($activity_slug !== '') {
        $mappings[] = $activity_slug;
        $mappings[] = 'index.php?page=' . $activity_slug;
        $mappings[] = 'admin.php?page=' . $activity_slug;
    }

    foreach (array_unique(array_filter($mappings, static function ($slug): bool {
        return is_string($slug) && $slug !== '';
    })) as $slug) {
        $_wp_real_parent_file[$slug] = $site_slug;
        $_parent_pages[$slug] = $site_slug;
    }
}
add_action('admin_menu', 'meza_register_site_settings_real_parent_mappings', PHP_INT_MAX);
add_action('admin_menu_editor-menu_replaced', 'meza_register_site_settings_real_parent_mappings', PHP_INT_MAX);

function meza_get_business_settings_menu_slug(): string
{
    return 'meza-business-settings';
}

function meza_get_site_settings_menu_slug(): string
{
    return 'meza-site-settings';
}

function meza_get_integrations_settings_menu_slug(): string
{
    return 'meza-integrations-settings';
}

function meza_get_ai_agents_menu_slug(): string
{
    return 'meza-ai-agents';
}

function meza_get_ai_agents_menu_capability(): string
{
    $capability = apply_filters('meza_ai_agents_menu_capability', 'manage_options');
...
function meza_can_access_ai_agents_menu($user = null): bool
{
    $capability = meza_get_ai_agents_menu_capability();
...
function meza_register_ai_agents_submenu_page(
    string $page_title,
    string $menu_title,
    string $capability,
    string $menu_slug,
    ...
    global $meza_ai_agents_submenu_pages;

    if (!is_array($meza_ai_agents_submenu_pages ?? null)) {
        $meza_ai_agents_submenu_pages = [];
    }

    static $registration_order = 0;
    $registration_order++;

    $meza_ai_agents_submenu_pages[$menu_slug] = [
...
function meza_get_registered_ai_agents_submenu_pages(): array
{
    global $meza_ai_agents_submenu_pages;
    ...
    $pages = array_values(array_filter($meza_ai_agents_submenu_pages, static function ($page): bool {
        return is_array($page)
            && is_callable($page['callback'] ?? null)
            && trim((string) ($page['page_title'] ?? '')) !== ''
            && trim((string) ($page['menu_title'] ?? '')) !== ''
            && trim((string) ($page['capability'] ?? '')) !== ''
            && trim((string) ($page['menu_slug'] ?? '')) !== '';
    }));

    usort($pages, static function (array $left, array $right): int {
        $left_position = is_int($left['position'] ?? null) ? (int) $left['position'] : PHP_INT_MAX;
        $right_position = is_int($right['position'] ?? null) ? (int) $right['position'] : PHP_INT_MAX;
...
function meza_get_ai_agents_first_accessible_submenu_page(): ?array
{
    foreach (meza_get_registered_ai_agents_submenu_pages() as $page) {
...
function meza_render_ai_agents_menu_page(): void
{
    if (!meza_can_access_ai_agents_menu(wp_get_current_user())) {
        wp_die(esc_html__('Sorry, you are not allowed to access this page.'));
    }

    $page = meza_get_ai_agents_first_accessible_submenu_page();
    if ($page !== null) {
        wp_safe_redirect(admin_url('admin.php?page=' . (string) $page['menu_slug']));
        exit;
    }

    wp_die(esc_html__('No AI agent admin pages are registered yet.'));
}

function meza_register_ai_agents_top_level_menu(): void
{
    do_action('meza_register_ai_agents_admin_pages', meza_get_ai_agents_menu_slug());

    $pages = meza_get_registered_ai_agents_submenu_pages();
    if ($pages === [] || !meza_can_access_ai_agents_menu(wp_get_current_user())) {
        return;
    }
    ...
    add_menu_page(
        'AI Agents',
        'AI Agents',
        meza_get_ai_agents_menu_capability(),
        meza_get_ai_agents_menu_slug(),
        'meza_render_ai_agents_menu_page',
        'dashicons-superhero',
        60
    );
    ...
    foreach ($pages as $page) {
        add_submenu_page(
            ...
            $page['callback'],
            is_int($page['position'] ?? null) ? (int) $page['position'] : null
        );
    }
}
add_action('admin_menu', 'meza_register_ai_agents_top_level_menu', 1);
add_action('admin_menu_editor-menu_replaced', 'meza_register_ai_agents_top_level_menu', 1);

function meza_can_access_site_settings($user = null): bool
{
    $capability = function_exists('meza_access_site_settings_capability')
...
function meza_register_business_settings_top_level_menu(): void
{
    if (!meza_can_access_business_settings(wp_get_current_user())) {
        return;
    }
...
function meza_register_site_settings_top_level_menu(): void
{
    ...
    add_menu_page(
        'Site',
        'Site',
        function_exists('meza_access_site_settings_capability')
            ? meza_access_site_settings_capability()
            : 'meza_access_site_settings',
        meza_get_site_settings_menu_slug(),
        '__return_null',
...
add_action('admin_init', function (): void {
    ...
    $page = isset($_GET['page']) ? sanitize_key((string) wp_unslash($_GET['page'])) : '';
    if ($page !== meza_get_business_settings_menu_slug() || !meza_can_access_business_settings(wp_get_current_user())) {
        return;
    }
    ...
    wp_safe_redirect(admin_url(meza_get_shared_project_acf_options_page_menu_slug('business-information')));
    exit;
}, 1);
...
add_action('admin_init', function (): void {
    ...
    $page = isset($_GET['page']) ? sanitize_key((string) wp_unslash($_GET['page'])) : '';
    if ($page !== meza_get_site_settings_menu_slug() || !meza_can_access_site_settings(wp_get_current_user())) {
        return;
    }
...
add_filter('parent_file', static function ($parent_file) {
    if (!is_admin() || !meza_can_access_site_settings(wp_get_current_user())) {
        return $parent_file;
    }

    $target_slug = meza_get_site_settings_submenu_target_slug();
    if ($target_slug === '') {
        return $parent_file;
    }

    return meza_get_site_settings_menu_slug();
}, PHP_INT_MAX - 2);
...
add_action('admin_init', function (): void {
    ...
    $page = isset($_GET['page']) ? sanitize_key((string) wp_unslash($_GET['page'])) : '';
    if ($page !== meza_get_integrations_settings_menu_slug() || !meza_can_access_integrations_settings(wp_get_current_user())) {
        return;
    }
...
function meza_position_business_settings_menu(): void
{
    if (!meza_can_access_business_settings(wp_get_current_user())) {
        return;
    }
    ...
    foreach ($menu as $index => $item) {
        ...
        $slug = (string) ($item[2] ?? '');
        if ($slug === meza_get_business_settings_menu_slug()) {
    ...
    if ($business_index === null || $appearance_index === null) {
        return;
    }

    $business_item = $menu[$business_index];
    unset($menu[$business_index]);
    $menu = array_values($menu);

    foreach ($menu as $index => $item) {
        if (is_array($item) && ((string) ($item[2] ?? '')) === 'themes.php') {
            array_splice($menu, $index, 0, [$business_item]);
            break;
        }
    }
}

function meza_position_site_settings_menu(): void
{
    if (!meza_can_access_site_settings(wp_get_current_user())) {
        return;
    }
    ...
    $site_item = $menu[$site_index];
    unset($menu[$site_index]);
    $menu = array_values($menu);

    if (count($menu) >= 1) {
        array_splice($menu, 1, 0, [$site_item]);
        return;
    }
...
function meza_position_integrations_settings_menu(): void
{
    if (!meza_can_access_integrations_settings(wp_get_current_user())) {
        return;
    }
    ...
    foreach ($menu as $index => $item) {
        ...
        $slug = (string) ($item[2] ?? '');
        if ($slug === meza_get_integrations_settings_menu_slug()) {
            $integrations_index = $index;
        }
    ...
    if ($integrations_index === null) {
        return;
    }

    $integrations_item = $menu[$integrations_index];
    unset($menu[$integrations_index]);
    $menu = array_values($menu);

    if ($plugins_index !== null) {
        foreach ($menu as $index => $item) {
            if (is_array($item) && ((string) ($item[2] ?? '')) === 'plugins.php') {
                array_splice($menu, $index + 1, 0, [$integrations_item]);
                return;
            }
        }
    }
    ...
    if ($users_index !== null) {
        foreach ($menu as $index => $item) {
            if (is_array($item) && ((string) ($item[2] ?? '')) === 'users.php') {
                array_splice($menu, $index, 0, [$integrations_item]);
                return;
            }
        }
    }

    $menu[] = $integrations_item;
}
...
function meza_remove_integrations_settings_menu_for_non_admins(): void
{
    ...
    if (is_array($menu)) {
        $menu = array_values(array_filter($menu, static function ($item): bool {
            return !is_array($item) || ((string) ($item[2] ?? '')) !== meza_get_integrations_settings_menu_slug();
        }));
    }

    if (is_array($submenu)) {
        unset($submenu[meza_get_integrations_settings_menu_slug()]);
    }
}
add_action('admin_menu', 'meza_remove_integrations_settings_menu_for_non_admins', PHP_INT_MAX);
add_action('admin_menu_editor-menu_replaced', 'meza_remove_integrations_settings_menu_for_non_admins', PHP_INT_MAX);
add_action('admin_head', 'meza_remove_integrations_settings_menu_for_non_admins', PHP_INT_MAX);
...
function meza_restore_profile_admin_menu_items(): void
{
    global $menu, $submenu;

    $user = wp_get_current_user();
    if (!($user instanceof WP_User) || !$user->exists() || !user_can($user, 'read')) {
        return;
    }
    ...
    $has_users_parent = false;
    if (is_array($menu)) {
        foreach ($menu as $item) {
            if (is_array($item) && ((string) ($item[2] ?? '')) === 'users.php') {
                $has_users_parent = true;
                break;
            }
        }
    }

    $has_profile_submenu = false;
    foreach ($submenu['users.php'] as $item) {
        if (is_array($item) && ((string) ($item[2] ?? '')) === 'profile.php') {
            $has_profile_submenu = true;
            break;
        }
    }

    if ($has_users_parent && !$has_profile_submenu) {
        $submenu['users.php'][] = [
            'Profile',
            'read',
            'profile.php',
    ...
    if (!isset($submenu['profile.php']) || !is_array($submenu['profile.php'])) {
        $submenu['profile.php'] = [];
    }

    foreach ($submenu['profile.php'] as $item) {
        if (is_array($item) && ((string) ($item[2] ?? '')) === 'profile.php') {
            return;
        }
    }
...
function meza_run_tail_admin_menu_mutations(): void
{
    ...
    meza_remove_stale_product_admin_menu_items();
    meza_restore_expected_post_type_archive_submenus();
